create or replace function public.merge_update_jsonb(target jsonb, path text[], object jsonb) returns jsonb as $$
declare
  i int;
  key text;
  value jsonb;
...
create or replace function public.merge_update_jsonb(table_name text, column_name text, row_id integer, object jsonb) returns jsonb as $$
declare
  target jsonb;
begin
